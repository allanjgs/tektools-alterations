...
const InputTokenTotal = ({ onAddQuantity }) => {
  ...
  const handleDecrement = () => {
    setQuantity(prevQuantity => Math.max(prevQuantity - 1000, 0));
  };

  const handleIncrement = () => {
    setQuantity(prevQuantity => Math.min(prevQuantity + 1000));
  };
    ...
    <div className='flex flex-col gap-y-1 gap-x-5 text-base font-thin'>
      ...
      <div className='flex flex-row gap-x-5'>
        <div className='flex border rounded-lg bg-accent'>
          ...
          <Input type="text" value={quantity} onChange={handleChangeQuantity} placeholder="Exemple: 1000000" className='text-center px-0 rounded-none border-none'  />
